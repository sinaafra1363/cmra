...
class BackupDialog(QDialog):
    def __init__(self, projects, parent=None):
        ...
        self.selected_projects_display = QTextEdit()
        self.selected_projects_display.setReadOnly(True)
        selected_projects_layout.addWidget(self.selected_projects_display)
        main_layout.addWidget(selected_projects_groupbox)
        
        self.project_list.itemChanged.connect(self.update_selected_projects_display)
        
        # بخش انتخاب محل ذخیره
        location_groupbox = QGroupBox("محل ذخیره")
        location_layout = QHBoxLayout(location_groupbox)
    ...
    def update_selected_projects_display(self, item):
        ...
        for i in range(self.project_list.count()):
            current_item = self.project_list.item(i)
            if current_item.checkState() == Qt.Checked:
                checked_projects.append(current_item.text())
        
        self.selected_projects_display.setText("\n".join(checked_projects))
    ...
    def accept(self):
        ...
        for i in range(self.project_list.count()):
            ...
            if item.checkState() == Qt.Checked:
                self.selected_projects.append(item.text())
    ...
    def get_selected_data(self):
        return self.selected_projects, self.save_path
    ...
    def get_stylesheet(self):
        return """
            ...
            QListWidget::indicator:checked {
                ...
                image: url(:/icons/icons/check.png); /* Example for an image checkmark */
            }
...
class Dashboard(QMainWindow):
    ...
    def backup_data(self):
        ...
        if backup_dialog.exec() == QDialog.Accepted:
            selected_projects, save_path = backup_dialog.get_selected_data()
            if selected_projects and save_path:
                QMessageBox.information(self, "پشتیبان‌گیری", 
                                       f"عملیات پشتیبان‌گیری برای پروژه‌های زیر آغاز شد:\n{', '.join(selected_projects)}\n"
                                       f"محل ذخیره: {save_path}")
            else:
                QMessageBox.warning(self, "خطا", "عملیات پشتیبان‌گیری لغو شد.")
